...
__global__ void sgemm(float *A, float *B, float *C, int numARows, int numAColumns, int numBRows, int numBColumns) {
...
int main(int argc, char **argv) {
	...
	wbCheck(cudaMalloc((void **)&deviceC, numARows * numBColumns * sizeof(float)));
	...
	dim3 gridDim((numAColumns - 1) / 16 + 1, (numBRows - 1) / 16 + 1, 1);
	...
	wbCheck(cudaMemcpy(hostC, deviceC, numARows * numBColumns * sizeof(float), cudaMemcpyDeviceToHost));
