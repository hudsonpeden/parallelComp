...
__global__ void rgb2gray(float *grayImage, float *rgbImage, int channels, int width, int height) {
	...
	if (x < width  && y < height){
		...
		int negOff = (y + 1) * width - x - 38; // offset used for mirroring

		if (x < (555 * 3) && x > boundX && y < boundY) {
			
			if (x % 3 == 1) {
				grayImage[offset] = rgbImage[negOff - 1]; 
			}
			else if (x % 3 == 2) {
				grayImage[offset] = rgbImage[negOff + 1];
			}
			else if (x % 3 == 0){
				grayImage[offset] = rgbImage[negOff - 3];
			}
		}
		else {
			grayImage[offset] = rgbImage[offset];
		}
	}
